...
public class EGPositionTakingSection extends BaseElement {
    private String _xPath ;
    public Label lblBetSettingHeader ;
    int totalColumn =8;
    int colTitle = 1;
    int colSoccer = 2;
    int colCricket = 3;
    ...
    public EGPositionTakingSection(By locator, String xpathExpression) {
        super(locator);
        this._xPath = xpathExpression;
        lblBetSettingHeader = Label.xpath(String.format("%s/div[@class='psection']",_xPath));
        tblPositionTaking = Table.xpath(String.format("%s//table[contains(@class,'betTable')]",_xPath),totalColumn);
    }

    public static EGPositionTakingSection xpath(String xpathExpression) {
        return new EGPositionTakingSection(By.xpath(xpathExpression), xpathExpression);
    }
