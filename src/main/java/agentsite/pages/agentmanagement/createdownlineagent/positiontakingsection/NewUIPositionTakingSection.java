...
//            Table tblPositionTaking = Table.xpath(String.format(positionTakingTableXpath, productCode), totalColumnExchange);
            ...
            Assert.assertEquals(lstSportHeader, LST_POSITION_TAKING_SPORT_HEADER, "FAILED! Position sport list is not shown correct");
            ...
            if(product.equalsIgnoreCase(AGConstant.LIVE_DEALER_ASIAN) || product.equalsIgnoreCase(AGConstant.LIVE_DEALER_EUROPEAN) || product.equalsIgnoreCase(AGConstant.PRAGMATIC)
                ...
                Assert.assertEquals(lstHeader, EVOLUTION_BET_PT_SETTING_HEADER,"FAILED! List position header does not display");
            } else if (product.equalsIgnoreCase(AGConstant.GAME_HALL)) {
                Assert.assertEquals(lstHeader, GAME_HALL_PT_SETTING_HEADER,"FAILED! List position header does not display");
            }
        }
