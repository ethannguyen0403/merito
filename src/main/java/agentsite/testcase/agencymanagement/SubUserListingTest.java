...
//        loginNewAccount(sosAgentURL, agentNewAccURL, subUsername, password, StringUtils.decrypt(environment.getSecurityCode()));
    ...
    public void Agent_SAD_Sub_User_Listing_3647() {
        ...
        log("Step 2. Click on Create button");
        SubUserPopup popup = page.subUserListing.openCreateUserPopup();
    ...
    public void Agent_SAD_Sub_User_Listing_3648(String password) throws Exception {
        ...
        String pwDecrypt = StringUtils.decrypt(password);
        SubUserListingPage page = agentHomePage.navigateSubUserListingPage();
        String userName = page.subUserListing.getActiveSubUser("", pwDecrypt);

        log("Step 2. Click on Edit for any account and only active Create Account permission");
        log("Step 3. Click Submit");
        page.subUserListing.editSubUser(userName, pwDecrypt, "Active", StringUtils.generateAlphabetic(4), StringUtils.generateAlphabetic(4), permissions);

        log("Verify 1.Verify Green check only display for Create Account and Account Balance column");
        Assert.assertTrue(page.subUserListing.isSubUserPermissionSettingCorrect(userName, permissions), "FAILED the list permission is incorrect");
        page.logout();
        Thread.sleep(2000);
        ...
        log("Verify 2. Verify there are menu is available when login sub account\n" +
                ...
                "- Create User");
        loginNewAccount(sosAgentURL, agentNewAccURL, userName, password, StringUtils.decrypt(environment.getSecurityCode()));
    ...
    public void Agent_SAD_Sub_User_Listing_3650(String password) throws Exception {
        ...
        log("3. Click Submit");
        page.subUserListing.editSubUser(userName, "", "Active", StringUtils.generateAlphabetic(4), StringUtils.generateAlphabetic(4), permissions);
