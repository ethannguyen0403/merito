...
public class DepositTest extends BaseCaseTest {
    ...
    public void DepositWithdrawal_Deposit_3621() {
        ...
        DepositPopup popup = (WithdrawalPopup) page.depositWithdraw.action(DepositWithdraw.Actions.DEPOSIT, userCode);
    ...
    public void DepositWithdrawal_Deposit_3622() {
        ...
        log("Step 2. Open Deposit popup of an account " + userCode);
        DepositToPopup popup = (DepositToPopup) page.depositWithdraw.action(DepositWithdraw.Actions.DEPOSIT, userCode);
    ...
    public void Agent_AM_DepositWithdrawal_Deposit_724() {
        ...
        List<String> userInfoBEfore = page.depositWithdraw.getRowContainsUsercode(userCode);
        String expectedAmountAfterDeposit = String.format(Locale.getDefault(), "%,.2f", Double.valueOf(userInfoBEfore.get(page.depositWithdraw.colAvailableBalance - 1).replaceAll(",", "").toString()) + amountDeposit);
        ...
        List<String> userInfoAfter = page.depositWithdraw.getRowContainsUsercode(userCode);

        log("Verify 3. Verify available balance of deposit account is updated");
        Assert.assertEquals(userInfoAfter.get(page.depositWithdraw.colAvailableBalance - 1), expectedAmountAfterDeposit,
                String.format("FAILED! Available Balance of account %s expected is %s but found %s", userCode, expectedAmountAfterDeposit, userInfoAfter.get(page.depositWithdraw.colAvailableBalance - 1)));
    ...
    public void Agent_AM_DepositWithdrawal_Deposit_725() throws InterruptedException {
        ...
        List<ArrayList<String>> listMemberInfo = page.depositWithdraw.getMemberInfo(n);

        log("Step  2. Check on more than 1 account and click on Deposit button");
        page.depositWithdraw.selectUser(listMemberInfo);
        DepositPopup popup = page.depositWithdraw.openDepositPopup();
        ...
        List<ArrayList<String>> listMemberInfoAfter = page.depositWithdraw.getMemberInfo(n);


        log("Verify 2. Verify Balance is updated correctly");
        Assert.assertTrue(page.depositWithdraw.verifyBalanceUpdated(depositAmount, loginAccBalance, listMemberInfo, listMemberInfoAfter, true, DepositWithdraw.Actions.SUCCESS_ICON), "FAILED! Balance value or status not correct after deposit");
        log("INFO: Executed completely");
    }
    ...
    public void Agent_AM_DepositWithdrawal_Deposit_727() {
        ...
        List<ArrayList<String>> mainBalanceInfoExpected = page.depositWithdraw.calculateMainAccountInfo(mainBalanceInfo, depositAmount, true);
        Assert.assertEquals(page.depositWithdraw.getLoginAccountBalanceInfo(), mainBalanceInfoExpected, "FAILED! Main account balance info is not match after deposit");

        log("Verify  3. Verify downline balance info");
        List<ArrayList<String>> listMemberInfoExpected = page.depositWithdraw.calculateDownlineBalanceInfo(listMemberInfo, depositAmount, true, false);
        Assert.assertEquals(page.depositWithdraw.getMemberInfo(1), listMemberInfoExpected, "FAILED! Members List balance is not match with the expected");
        log("INFO: Executed completely");
    }
    ...
    public void Agent_AM_DepositWithdrawal_Deposit_728() {
        ...
        List<ArrayList<String>> mainBalanceInfoExpected = page.depositWithdraw.calculateMainAccountInfo(mainBalanceInfo, totalDepositAmount, true);
    ...
    public void Agent_AM_DepositWithdrawal_Deposit_729() throws InterruptedException {
        ...
        Assert.assertTrue(page.depositWithdraw.verifyUpdateStatus(listMemberInfo, false), "FAILED! ERROR check on Update Status column is not display as expected");
    ...
    public void Agent_AM_DepositWithdrawal_Deposit_730() {
        ...
        page.depositWithdraw.selectAllCheckBoxes();

        log("Verify 1. Verify Deposit/withdraw link is disabled/enabled");
        Assert.assertTrue(page.depositWithdraw.verifyDepositWithDrawLink(false), String.format("ERROR! Deposit/withdraw link not disable when select all checkbox"));
    ...
    public void Agent_AM_DepositWithdrawal_Deposit_3624(String memberAccount, String password) throws Exception {
        ...
        page.depositWithdraw.deposit(userCode, String.format("%.2f", depositAmount), String.format("Deposit TC13 deposit for Player " + userCode + " with amount " + depositAmount), true, true);
    ...
    public void Agent_AM_DepositWithdrawal_Deposit_3625(String memberAccount, String password) throws Exception {
        ...
        page.depositWithdraw.withdraw(userCode, String.format("%.2f", withdraw), String.format("Deposit TC14 withdraw for Player " + userCode + " with amount " + withdraw), true, true);
