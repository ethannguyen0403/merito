...
import agentsite.pages.all.agentmanagement.FollowBetsPage;
import agentsite.pages.all.agentmanagement.followbets.GroupDetailsPopup;
...
import static agentsite.common.AGConstant.HomePage.FOLLOW_BETS;

public class FollowBetsTest extends BaseCaseMerito {
    /**
    ...
    public void Agent_AM_Follow_Bets_001(){
        ...
        FollowBetsPage page = agentHomePage.clickSubMenu(AGENCY_MANAGEMENT, FOLLOW_BETS, FollowBetsPage.class);
    ...
    public void Agent_AM_Follow_Bets_009(String accountToBet,String controlBlockingAccount){
        ...
        try {
            ...
            Assert.assertEquals(addAgentPlayerLst.get(0).get(page.colLoginID - 1), controlBlockingAccount, "FAILED! Agent account added not match");
            ...
            Assert.assertEquals(confirmMsg, String.format("Confirm to remove user %s ?", addAgentPlayerLst.get(0).get(page.colUsername-1)), "FAILED! Confirm remove agent/player is incorrect");
