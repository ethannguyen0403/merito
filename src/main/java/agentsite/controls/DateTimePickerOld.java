 ...
 */
public class DateTimePickerOld extends BaseElement {
    ...
    private DateTimePickerOld(TextBox txtBox, By locator, String xpath) {
        super(locator);
        txtDate = txtBox;
        _xpath = xpath;
        lblMonth = Label.xpath(String.format("%s//%s", _xpath, "button[@class='owl-dt-control-content owl-dt-control-button-content']"));
        lblYear = Label.xpath(String.format("%s//%s", _xpath, "button[@class='current']"));
        btnPrevious = Button.xpath(String.format("%s//%s", _xpath, "button[@aria-label='Previous month']"));
        btnNext = Button.xpath(String.format("%s//%s", _xpath, "button[@aria-label='Next month']]"));
        btnCurrent = Button.xpath(String.format("%s//%s", _xpath, "button[@class='current']"));
        lblMonthYear = Label.xpath(String.format("%s//%s", _xpath, "div[@class='owl-dt-calendar-control-content']//span[contains(@class,'owl-dt-control-content owl-dt-control-button-content')]"));
        tblCalender = Table.xpath(String.format("%s//%s", _xpath, "table[contains(@class, 'owl-dt-calendar-table owl-dt-calendar-month-table')]"), 7);
        tblCalenderMonth = Table.xpath(String.format("%s//%s", _xpath, "table[@class='owl-dt-calendar-table owl-dt-calendar-year-table']"), 3);
        btnSet = Button.xpath(String.format("%s//div[contains(@class,'owl-dt-container-button')]//button[2]", _xpath));
        btnCancel = Button.xpath(String.format("%s//div[contains(@class,'owl-dt-container-button')]//button[1]", _xpath));
    }

    public static DateTimePickerOld xpath(TextBox txtBox, String xpathExpression) {
        return new DateTimePickerOld(txtBox, By.xpath(xpathExpression), xpathExpression);
    }

    public void setTime(String time, String fomat) {
        selectDate(time, fomat);
        btnSet.click();
    }
        ...
        while (i <= rows) {
            ...
            for (int j = 2; j <= tblCalender._columnNumber + 1; j++) {
                ...
                if (Objects.nonNull(e)) {
                    ...
                    if (Label.xpath(e.getLocator().toString().split("By.xpath: ")[1] + "//span[@class='owl-dt-calendar-cell-content owl-dt-calendar-cell-out']").isDisplayed()) {
                        continue;
                    }
                    if (Label.xpath(e.getLocator().toString().split("By.xpath: ")[1] + "//span[@class='owl-dt-calendar-cell-content']").isDisplayed()) {
                        e.click();
    ...
    public void selectDate(String input, String format) {
        txtDate.click();
        String currentMonthYear = lblMonthYear.getText().trim();
