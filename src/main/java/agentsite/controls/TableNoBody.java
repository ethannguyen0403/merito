 ...
 */
public class TableNoBody extends BaseElement {
	...
	public TableNoBody(By locator, String xpathExpression, String url, int columnNumber, String header, String json) {
		super(locator);
	...
	public static TableNoBody xpath(String xpathExpression, int columnNumber) {
		return new TableNoBody(By.xpath(xpathExpression), xpathExpression, null, columnNumber, null, null);
	}

	public static TableNoBody xpath(String xpathExpression, String url, int columnNumber, String header, String json) {
		return new TableNoBody(By.xpath(xpathExpression), xpathExpression, url, columnNumber, header, json);
	}
	...
	public List<ArrayList<String>> getRows(boolean isMoved){
		...
		String rowXpath = String.format("%s%s", this._xpathTable, "/tr[%s]");
		int i = 1;
	...
	public BaseElement getControlOfCell(int tBodyOrder, int columnOrder, int rowOrder, String subTag){
		...
		String cellXpath;
		if (subTag == null){
			cellXpath = String.format("%s%s%s", this._xpathTable, String.format("//tr[%s]/", rowOrder), String.format("td[%s]", columnOrder));
		} else {
			cellXpath = String.format("%s%s%s//%s", this._xpathTable, String.format("//tr[%s]/", rowOrder), String.format("td[%s]", columnOrder), subTag);
		}
	...
	public int getNumberOfRows(boolean isCountHeaderRow, boolean isMoving){
		...
		/*String rowXpath = String.format("%s%s", this._xpathTable, "//tr");
		...
		String rowXpath = String.format("%s%s", this._xpathTable, "//tr[%d]");
		int i = 1;
		while (true){
...
//	}
	...
	public String getControlxPathBasedValueOfDifferentColumnOnRow(String value, int tBodyOrder, int columnOrder, int beginRow, String subTag, int columnOfControl, String subTagOfControl, boolean isMoved, boolean isDifferentText) {
		...
		int i = beginRow;
		String hasBodyXpath = String.format("%s%s", this._xpathTable, "//tr[%s]/");
	...
	private BaseElement getControlOnRow(String value, int tbodyOrder, int beginColumn, int rowOrder, String subTag, boolean isMoved, boolean isDifferentText){
		...
		if (subTag == null || subTag.isEmpty()){
			cellXpath = String.format("%s%s%s", this._xpathTable, String.format("//tr[%s]/", rowOrder), "td[%s]");
		} else {
			cellXpath = String.format("%s%s%s//%s", this._xpathTable, String.format("//tr[%s]/", rowOrder), "td[%s]", subTag);
		}
		while(true) {
			if (i > _columnNumber) {
				return null;
