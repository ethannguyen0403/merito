...
public class MyBetPageTest extends BaseCaseMerito {
    ...
    public void MyBetPage_TC001(String timeZone){
        ...
        page.ddbProduct.selectByVisibleText(DDB_PRODUCT_FILTER.get("Exchange"));
        ...
        Assert.assertEquals(page.lblProduct.getText(), DDB_PRODUCT_FILTER.get("Exchange"),String.format("ERROR! Expected product lable is %s but display %s", DDB_PRODUCT_FILTER.get("Exchange"), page.lblProduct.getText()));
        Assert.assertEquals(page.lblOrderType.getText(),ORDER_TYPE, String.format("ERROR! Expected Start Date but found %s",page.lblOrderType.getText()));
        Assert.assertEquals(page.lblStartDate.getText(),START_DATE, String.format("ERROR! Expected Start Date but found %s",page.lblStartDate.getText()));
        Assert.assertEquals(page.lblEndDate.getText(),END_DATE, String.format("ERROR! Expected End Date but found %s",page.lblEndDate.getText()));
        Assert.assertEquals(page.lblNote.getText(),String.format(NOTES,timeZone), String.format("ERROR! Current Note label shows %s",page.lblNote.getText()));
