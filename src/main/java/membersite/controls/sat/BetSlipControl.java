...
public class BetSlipControl extends BaseElement {
	...
	private Label lblProfitLiability;
	...
	private BetSlipControl(By locator, String xpath) {
		super(locator);
		_xpath = xpath;
		//btnPlaceBet = Button.xpath("//button[contains(@class,'button-primary')]");
		//btnClearAll = Button.xpath("//button[contains(@class,'btn-cancel')]");
		lblSelection = Label.xpath(lblSelectionNameXPath);
		txtOdds = TextBox.xpath(txtOddsXPath);
		txtStake = TextBox.xpath(txtStakeXPath);
		lblProfitLiability = Label.xpath("//span[contains(@class,'bet-slip-profit')]");
		//lblAccepting = Label.xpath("//div[@class='loading-text']");
	}
	...
	public boolean isStakeDisplayAsClickingOnFastButton()
	{
		...
		for(int i =0, n= btnQuickStakes.getWebElements().size(); i<n; i++)
		{
			...
			if(!actual.equals(txtStake.getAttribute("value"))){
				logEndAction(String.format("ERROR: Expected fast button is %s but found %s ", element.getAttribute("value"), txtStake.getAttribute("value")));
				return false;
			}
		}
		return true;
