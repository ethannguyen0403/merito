 ...
 * @date Dec/31/2019.
 */
public class ATable extends BaseElement {
    ...
    private ATable(By locator, String xpathExpression, String url, int columnNumber, String header, String json) {
		super(locator);
	...
	public static ATable xpath(String xpathExpression, int columnNumber) {
		return new ATable(By.xpath(xpathExpression), xpathExpression, null, columnNumber, null, null);
	}
	...
	public List<ArrayList<String>> getRows(boolean isMoved){
		...
		String rowXpath = String.format("%s%s", this._xpathTable, "/div[contains(@class,'body')]/div[contains(@class,'row')][%s]");
		int i = 1;
		while(true) {
	...
	public List<String> getColumn(int columnOrder, int limit, boolean isMoved){
		...
		int i = 1;
		String cellXpath = String.format("%s%s%s", this._xpathTable, "/div[contains(@class,'body')]/div[contains(@class,'row')][%s]/", String.format("div[%s]", columnOrder));
		while(true) {
	...
	public List<WebElement> getWebElementsOfCell(int columnOrder, int rowNumber, String tagName, boolean isHeader, boolean isScrolled , boolean isAlignedToTop){
		...
		if (isHeader) {
			...
			rowXpath = String.format("%s%s", this._xpathTable, "/div[contains(@class,'body')]/div[%s]");
	...
	public BaseElement getControlOfCell(int tBodyOrder, int columnOrder, int rowOrder, String subTag){
		...
		String cellXpath;
		if (subTag == null){
			cellXpath = String.format("%s%s%s", this._xpathTable, String.format("/div[contains(@class,'body')][%s]/div[contains(@class,'row')][%s]/", tBodyOrder, rowOrder), String.format("div[%s]", columnOrder));
		} else {
			cellXpath = String.format("%s%s%s//%s", this._xpathTable, String.format("/div[contains(@class,'body')][%s]/div[contains(@class,'row')][%s]/", tBodyOrder, rowOrder), String.format("div[%s]", columnOrder), subTag);
		}
	...
	public int getNumberOfRows(boolean isCountHeaderRow, boolean isMoving){
		...
		String rowXpath = String.format("%s%s", this._xpathTable, "/div[contains(@class,'body')]/div[%d]");
		int i = 1;
		while (true){
...
//	}
	...
	private List<ArrayList<String>> getRows(int limitedRow, boolean isHeaderGot, boolean isMoved){
		...
		String rowXpath = String.format("%s%s", this._xpathTable, "/div[contains(@class,'body')]/div[%s]");
		int i = 1;
		while(true){
			ARow row = ARow.xpath(String.format(rowXpath, i));
			if(!row.isDisplayedShort(timeOutShort)){
